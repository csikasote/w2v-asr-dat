...
class ModelArguments:
    ...
    do_stable_layer_norm: Optional[bool] = field(
            default=True,
            metadata={
                "help": "Whether to use the stable layer norms"
                },
            )
    use_adapter_attn_layer: Optional[bool] = field(
            default=False,
            metadata={"help": "Whether to insert the adapters after the MHSA or not. False implies not while true means insert."},
            )
    use_adapter_ffn_layer: Optional[bool] = field(
            default=True,
            metadata={"help": "Whether to insert the adapter after the 2 x feedword networks or not. False implies not while True means insert"},
            )
...
class DataTrainingArguments:
    ...
    label_column_name: str = field(
        default="domain_label", metadata={"help": "The name of the dataset column containing the labels. Defaults to 'domain_label'"}
    )
...
@dataclass
class DataCollatorCTCWithPaddingAndDAT:
    processor: AutoProcessor
    padding: Union[bool, str] = "longest"
    pad_to_multiple_of: Optional[int] = None
    pad_to_multiple_of_labels: Optional[int] = None
    ...
    def __call__(self, features: List[Dict[str, Union[List[int], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
        ...
        input_features = [{"input_values": feature["input_values"]} for feature in features]
        label_features = [{"input_ids": feature["labels"]} for feature in features]
        domain_labels = [{"domain_label": feature["domain_label"]} for feature in features]
        ...
        labels = labels_batch["input_ids"].masked_fill(labels_batch.attention_mask.ne(1), -100)
        batch["labels"] = labels
        batch["domain_label"] = torch.tensor(domain_labels)
...
class CTCTrainerWithDAT(Trainer):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.domain_classifier = DomainClassifier(input_dim=1024, hidden_dim=256, num_domains=2)
    ...
    def training_step(self, model, inputs, num_items_in_batch=None):
        ...
        with self.compute_loss_context_manager():
            ...
            hidden_states = self.grl(hidden_states.mean(dim=1))  # Apply GRL
            domain_logits = self.domain_classifier(hidden_states)
            domain_labels = inputs["domain_label"]
            domain_loss = self.domain_criterion(domain_logits, domain_labels)

            # Combine losses
            loss = ctc_loss + domain_loss
...
def main():
    ...
    config.update(
        {
            ...
            "do_stable_layer_norm": model_args.do_stable_layer_norm,
            "use_adapter_attn_layer": model_args.use_adapter_attn_layer,
            "use_adapter_ffn_layer": model_args.use_adapter_ffn_layer,
        }
    )
    ...
    def prepare_dataset(batch):
        ...
        # encode targets
        batch["labels"] = tokenizer(batch["target_text"]).input_ids
        batch["domain_label"] = batch["domain_label"]  # Ensure domain labels are included
        return batch
    ...
    data_collator = DataCollatorCTCWithPaddingAndDAT(processor=processor)

    # Initialize Trainer
    trainer = CTCTrainerWithDAT(
        model=model,
